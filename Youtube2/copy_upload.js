...
const upload = async (credentials, videos, puppeteerLaunch, messageTransport = defaultMessageTransport) => {
    ...
    try {
        messageTransport.debug("Trying to load account");
...
async function uploadVideo(videoJSON, messageTransport) {
    ...
    messageTransport.error("This is the uploadURL", uploadURL);
...
async function loadAccount(credentials, messageTransport, useCookieStore = true) {
    try {
        if (!fs_extra_1.default.existsSync(cookiesFilePath) || !useCookieStore)
            messageTransport.debug("Logging into account");
            await login(page, credentials, messageTransport, useCookieStore);
    }
    catch (error) {
        if (error.message === 'Recapcha found') {
            messageTransport.debug("Found Recapcha");
            if (browser) {
                await browser.close();
            }
            messageTransport.debug("Browser closed, throwing error because of recapcha");
            throw error;
        }
        // Login failed trying again to login
        try {
            messageTransport.debug("Trying to login again");
            await login(page, credentials, messageTransport, useCookieStore);
        }
        catch (error) {
            if (browser) {
            ...
            messageTransport.debug("Unable to login");
...
async function changeHomePageLangIfNeeded(localPage) {
    messageTransport.debug("Navigating to upload URL");
...
async function login(localPage, credentials, messageTransport, useCookieStore = true) {
    try {
        messageTransport.debug("Starting login process");
        var _a;
        messageTransport.debug("Navigating to upload URL");
        await localPage.goto(uploadURL);
        if (!useCookieStore) {
            try {
                ...
                messageTransport.debug("Not already logged in, need to login again");
             }
        }
        messageTransport.debug("Attempting to change login page language");
        await changeLoginPageLangIfNeeded(localPage);
        messageTransport.debug("Login page language changed (if needed)");
        const emailInputSelector = 'input[type="email"]';
        await localPage.waitForSelector(emailInputSelector);
        messageTransport.debug("Entering email address");
        await localPage.type(emailInputSelector, credentials.email, { delay: 50 });
        await localPage.keyboard.press('Enter');
        messageTransport.debug("Email entered, waiting for navigation");
    
        // check if 2fa code was sent to phone
        messageTransport.debug("Waiting for navigation after email entry");
        await localPage.waitForNavigation();
        messageTransport.debug("Navigation after email entry completed");
        messageTransport.debug("Starting 1000ms timeout");
        await localPage.waitForTimeout(1000);
        messageTransport.debug("Starting 1000ms timeout");
        ...
        try {
            await localPage.waitForNavigation();
            messageTransport.debug("Waiting timeout for 5000 now");
        ...
        messageTransport.debug("Login process completed successfully");
    } catch (error) {
        messageTransport.debug(`Login process failed: ${error.message}`);
        messageTransport.debug(`Error stack: ${error.stack}`);
        throw error;
    }
